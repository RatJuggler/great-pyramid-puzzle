...
void drawTilesOnFace(real offset, real scale, int rotate, string id, real offset_x, real offset_y, real offset_y2) {
    ...
    drawTilePosition(rotate(rotate) * (center + (0, offset_y2)), scale, rotate, id + "-1");
    drawTilePosition(rotate(rotate) * (center + (-offset_x, -offset_y)), scale, rotate, id + "-2");
    drawTilePosition(rotate(rotate) * (center + (0, 0)), scale, rotate - 60, id + "-3");
...
// Title
label("Tile positions for the Pocket puzzle.", (0, point(face_1, 0).y - 0.4), N, title_label);
