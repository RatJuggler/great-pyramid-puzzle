...
export class DisplayManager {
    ...
    private displayEmptyTilePosition(tpCenter: CenterPointData, tilePositionId: string): G {
    ...
    private displayEmptyFace(fData: FaceDisplayData): void {
        ...
        fData.tilePositions.forEach((tpData) => {
            ...
            fGroup.add(
                this.displayEmptyTilePosition(tpCenter, fData.name + '-' + tpData.id)
            );
        });
        // Outline the face.
    ...
    initialDisplay(): Svg {
        ...
        this.drawTriangle(startGroup, this._startCenter, this._scaleTile, DisplayManager.START_POSITION_COLOUR);
    ...
    display(tpChange: TilePositionChange) {
        ...
        switch (tpChange.type) {
            case "Empty":
                action = new EmptyTilePosition(this, tpChange);
                break;
            case "Final":
                action = new FinalTilePosition(this, tpChange as TileChange);
                break;
            case "Place":
                action = new PlaceTilePosition(this, tpChange as TileChange);
                break;
            case "Rotate":
                action = new RotateTilePosition(this, tpChange);
                break;
            case "Remove":
                action = new RemoveTilePosition(this, tpChange as TileChange);
                break;
            default:
