...
describe("TilePosition behaviour", function () {
    ...
    describe("when a new TilePosition is first created", function () {
        ...
        context("without joining to any other TilePositions", function () {
            ...
            it("should return the correct toString result", function () {
                const expectedToString = "TilePosition: XYZ, On Face: 1, Rotations: 0, Contains Tile: [Empty], Joins: ";
                expect(tilePosition.toString()).to.equal(expectedToString);
            });
            it("should fail the integrity check", function () {
                const expectedFailure =
                    [false, "Tile position joins not complete: TilePosition: XYZ, On Face: 1, Rotations: 0, Contains Tile: [Empty], Joins: "];
        ...
        context("and then joined to three other TilePositions", function () {
            ...
            it("should return the correct toString result", function () {
                const expectedToString =
                    "TilePosition: XYZ, On Face: 1, Rotations: 0, Contains Tile: [Empty], Joins: (XYZ-A->1-TP1-B)(XYZ-B->1-TP2-C)(XYZ-C->1-TP3-A)";
                expect(tilePosition.toString()).to.equal(expectedToString);
            });
    ...
    describe("if #join() is called to join one TilePosition to another", function () {
        ...
        context("with valid side names for the two different TilePositions to be joined", function () {
            ...
            it("should join the TilePositions in the direction given", function () {
                const tile1ExpectedToString = "TilePosition: TP1, On Face: 1, Rotations: 0, Contains Tile: [Empty], Joins: (TP1-A->1-TP2-B)";
            ...
            it("should not join the TilePositions in the opposite direction", function () {
                const tile2ExpectedToString = "TilePosition: TP2, On Face: 1, Rotations: 0, Contains Tile: [Empty], Joins: ";
    ...
    describe("if #setTile() is called to place a Tile (without using matching)", function () {
        ...
        context("and the TilePosition is empty", function () {
            ...
            it("and the rotation as 0", function () {
                expect(tilePosition.rotations).to.equal(0);
            })
        });
    ...
    describe("if #rotations() is called on a TilePosition", function () {
        ...
        context("with an invalid number of rotations", function () {
            ...
            it("should throw an error", function () {
                expect(function () {
                    tilePosition.rotations = 4;
                }).to.throw(Error, "Tile can only be rotated 0, 1 or 2 times!");
            });
            it("should throw an error", function () {
                expect(function () {
                    tilePosition.rotations = -3;
                }).to.throw(Error, "Tile can only be rotated 0, 1 or 2 times!");
            });
            it("should throw an error", function () {
                expect(function () {
                    tilePosition.rotations = 1.5;
                }).to.throw(Error, "Tile can only be rotated 0, 1 or 2 times!");
            });
        });
        ...
        context("with a valid number of rotations", function () {
            const tilePosition = new TilePosition("TP", "1");
            tilePosition.tile = TILE_1;
            tilePosition.rotations = 2;
            it("should set the rotations", function () {
                expect(tilePosition.rotations).to.equal(2);
            });
        });
    ...
    describe("if #rotate() is called on a TilePosition", function () {
        ...
        context("with a newly placed Tile", function () {
            const tilePosition = new TilePosition("TP", "1");
            tilePosition.tile = TILE_1;
            const result = tilePosition.rotate();
            it("should return true", function () {
                expect(result).to.be.true;
            });
            it("should track the Tile as being rotated once", function () {
                expect(tilePosition.rotations).to.equal(1);
            });
        });
    ...
    describe("if #tilesMatch() is called to check a Tile and there are three adjoining TilePositions", function () {
        ...
        context("and the three adjoining TilePositions each have a Tile, and all the sides match", function () {
            ...
            tilePosition2.tile = TILE_3;
            tilePosition3.tile = TILE_1;
            tilePosition3.rotate();
            tilePosition3.rotate();
            tilePosition4.tile = TILE_4;
            tilePosition4.rotate();
            const result = tilePosition1.tilesMatch();
            it("should return true", function () {
                expect(result).to.be.true;
            });
